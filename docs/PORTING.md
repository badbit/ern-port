...
- **Forms maximizados** (`raw.WindowState == "2"`): en el original arrancaban a pantalla completa (640×480 de la época) y su layout desborda el client_w/client_h guardado. El motor agranda el cliente al bounding box de los controles visibles (excluyendo Timers), con tope al tamaño de la pantalla actual.
- **TextBox multiline**: el `tk.Text` va empaquetado dentro de un Frame contenedor *colocado* con `place()`; toda operación de geometría/visibilidad (autofill de lectores, `set_prop visible/left/top`, `move_by`) debe actuar sobre ese contenedor (`meta["_outer"]`), no sobre el Text interno.
